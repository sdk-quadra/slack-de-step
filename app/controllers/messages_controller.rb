...
class MessagesController < ApplicationController
  ...
  def create
    ...
    members = Channel.find(params[:channel_id]).companions.map { |companion| companion.slack_user_id }

    members.each do |member|

      companion = Companion.find_by(slack_user_id: member)
      participation_datetime = companion.participations.where(companion_id: companion.id).where(channel_id: params[:channel_id]).map { |participation| participation.created_at }.first
